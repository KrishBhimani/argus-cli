...
def install(
    ...
    """Register argusd to start on login AND start it now."""
    _setup_logging()
    from .daemon import pidfile
    from .daemon.autostart import install_autostart, linger_hint
    from .daemon.process import wait_for_pidfile

    existing = pidfile.read(data_dir)
    already_running = existing is not None and pidfile.is_running(existing)

    # If already running, register autostart but DON'T start a second copy
    # (the service-manager start would trip the collision guard).
    try:
        msg = install_autostart(data_dir, start_now=not already_running)
    ...
    if already_running:
        typer.echo(
            f"daemon already running, PID {existing}; registered for autostart."
        )
    else:
        pid = wait_for_pidfile(data_dir)
        if pid is None:
            typer.echo(
                "Autostart registered, but the daemon did not come up yet — "
                "check ~/.argus/argusd.log.",
                err=True,
            )
        else:
...
def uninstall(
    ...
    """Stop argusd, deregister autostart, and remove its files."""
    _setup_logging()
    from .daemon.autostart import autostart_present, uninstall_autostart
    from .daemon.process import stop_daemon

    # Deregister + service-manager stop FIRST so a KeepAlive/Restart supervisor
    # doesn't resurrect the daemon after we kill it.
    if autostart_present():
        typer.echo(uninstall_autostart(data_dir))
    else:
        typer.echo("No autostart found.")

    # Also stop a manually-started (unsupervised) daemon, if any remains.
    if stop_daemon(data_dir):
        typer.echo("argusd stopped.")
    else:
        typer.echo("argusd was not running.")
