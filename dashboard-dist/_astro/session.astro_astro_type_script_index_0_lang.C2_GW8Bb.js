import{a as k,u as m,t as C,n as a,d as E,e as n}from"./format.DxC1NGYT.js";const $=Intl.DateTimeFormat().resolvedOptions().timeZone,f=(()=>{const t=new Date().toLocaleTimeString("en-US",{timeZoneName:"short"}).match(/[A-Z]{2,5}$/);return t?t[0]:""})(),u=s=>s?new Date(s).toLocaleString([],{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!1}):"—",b=s=>s?new Date(s).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!1}):"—";function M(s){const t="\0MK_OPEN\0",r="\0MK_CLOSE\0",l=s.replace(/<mark>/g,t).replace(/<\/mark>/g,r);return n(l).replace(new RegExp(t,"g"),"<mark>").replace(new RegExp(r,"g"),"</mark>")}function D(s){switch(s){case"user":return"you said";case"assistant":return"claude said";case"thinking":return"claude thinking";case"tool_result":return"tool output";default:return s}}const T=new URLSearchParams(location.search),h=T.get("id"),x=T.get("q")??"",g=document.getElementById("content");h?k.session(h).then(s=>{if(!s){g.innerHTML='<p class="empty">Session not found.</p>';return}const{session:t,turns:r}=s,l=t.total_fresh_input_tokens+t.total_output_tokens+t.total_cache_read_tokens+t.total_cache_write_tokens,o=t.metadata?.sub_agent_session_ids,w=t.agent_reported_cost_usd!=null?`<span style="color:var(--text-2);font-size:0.8rem;"> (agent reported: ${m(t.agent_reported_cost_usd)})</span>`:"";g.innerHTML=`
          ...
          <div class="card" style="margin-bottom:1rem;">
            ...
            <ul style="margin:0;padding-left:1.2rem;color:var(--text-1);font-size:0.88rem;">
              ${o.map(e=>`<li><a href="/session?id=${encodeURIComponent(e)}" class="mono">${n(e)}</a></li>`).join("")}
            </ul>
          </div>`:""}
        ...
        `;const p=document.getElementById("seg-q"),d=document.getElementById("seg-results"),v=document.getElementById("seg-count");let y;async function _(){const e=p.value.trim();if(!e){d.innerHTML="",v.textContent="";return}try{const c=await k.sessionTranscriptSearch(h,e,200);if(v.textContent=`${c.total} match${c.total===1?"":"es"}`,c.segments.length===0){d.innerHTML='<p class="empty" style="margin:0.5rem 0;">No matches in this session.</p>';return}const S=c.segments.slice().sort((i,L)=>Date.parse(i.timestamp)-Date.parse(L.timestamp));d.innerHTML=S.map(i=>`
              <div class="sd-seg-card">
                <div class="sd-seg-meta">
                  <span class="sd-role-pill role-${i.role}">${D(i.role)}</span>
                  <span style="margin-left:0.5rem;">${b(i.timestamp)}</span>
                </div>
                <div class="sd-seg-snippet">${M(i.snippet)}</div>
              </div>
